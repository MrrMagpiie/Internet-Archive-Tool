...
class DocumentCard(QFrame):
    ...
    def __init__(self, document: Document, stage: int):
        ...
        # Dynamically name it so QSS can color the chunk based on the stage
    ...
    def contextMenuEvent(self, event):
        ...
        # 2. Create the action and add your qtawesome trash icon
        remove_action = context_menu.addAction("Remove Document")
        delete_action = context_menu.addAction("Delete Document")
        
        # We use standard danger red here. If you pass your theme dictionary into 
        ...
        # 3. Show the menu at the exact mouse cursor position
        clicked_action = context_menu.exec(self.mapToGlobal(event.pos()))
        
        # 4. If they clicked delete, trigger the confirmation dialog
    ...
    def _confirm_deletion(self):
        ...
        reply = QMessageBox.question(
            self,
            "Confirm Deletion",
            f"Are you sure you want to permanently delete '{self.title}'?\n\nThis will remove it from the database and delete all associated files. \n this will not delete the original scans.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No # Defaults to No so hitting Enter cancels it
        )
        
        if reply == QMessageBox.StandardButton.Yes:
            # Emit the signal up to the Dashboard/View to handle the actual database call
            ticket = DatabaseTicket()
            self.delete_requested.emit(self.doc)
